...
sparcs['Length of Stay'] = pd.to_numeric(
    sparcs['Length of Stay'], errors='coerce')
sparcs['sparcs_los'] = sparcs['Length of Stay'].apply(
    lambda x: 'long' if x > 3 else 'short')
sparcs['sparcs_los'].value_counts()
sparcs.drop('Length of Stay', axis=1, inplace=True)
sparcs.columns

# just get values from sparcs_los
